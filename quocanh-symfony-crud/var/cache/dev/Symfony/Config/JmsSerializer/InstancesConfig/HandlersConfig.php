...
class HandlersConfig 
{
    ...
    /**
     * @default {"default_format":"Y-m-d\\TH:i:sP","default_timezone":"UTC","cdata":true}
    */
    ...
    */
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('datetime', $value)) {
            $this->_usedProperties['datetime'] = true;
            $this->datetime = new \Symfony\Config\JmsSerializer\InstancesConfig\Handlers\DatetimeConfig($value['datetime']);
            unset($value['datetime']);
        }
    
        if (array_key_exists('array_collection', $value)) {
            $this->_usedProperties['arrayCollection'] = true;
            $this->arrayCollection = new \Symfony\Config\JmsSerializer\InstancesConfig\Handlers\ArrayCollectionConfig($value['array_collection']);
            unset($value['array_collection']);
        }
    
        if (array_key_exists('symfony_uid', $value)) {
            $this->_usedProperties['symfonyUid'] = true;
            $this->symfonyUid = new \Symfony\Config\JmsSerializer\InstancesConfig\Handlers\SymfonyUidConfig($value['symfony_uid']);
